...
# George eliminating some of the series
AllTS_new %>% log() %>%
  dplyr::select("AA", "AF", "BB", "BE",
                "CA", "CD", "DC", "EC",
                "FB", "GA") %>%
...
AllTS_new %>% log() %>%
  dplyr::select("Bendigo Loddon",
                ...
                "Experience Perth",
                "Darwin") %>%
  rename("Tropical Nth QLD" = "Tropical North Queensland",
         "Perth" = "Experience Perth") %>%
  ts(start = c(1998, 1), frequency = 12) %>%
  autoplot() + ylab("log(Overnight trips)") + xlab("Time") +
  ggtitle("Regions") + theme(legend.position="bottom") -> Plot_regions_2

grid.arrange(arrangeGrob(Plot_regions_1),
             arrangeGrob(Plot_regions_2),
             ncol = 2) -> Plot_regions
