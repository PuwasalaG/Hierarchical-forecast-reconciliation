...
\begin{document}
	...
	\begin{abstract} 
		A geometric interpretation is developed for so-called {\em reconciliation} methodologies which are used to forecast time series that adhere to known linear constraints.  In particular, a general framework is established nesting many existing popular reconciliation methods within the class of {\em projections}.  This interpretation facilitates the derivation of novel results that explain why and how reconciliation via projection is guaranteed to improve forecasts with respect to a specific class of loss functions.  This result is later demonstrated empirically.  The geometric interpretation is also used to provide a new proof that forecast reconciliation results in unbiased forecasts as long as the initial so-called {\em base} forecasts are also unbiased.  Approaches for dealing with biased base forecasts are proposed and explored in an extensive empirical study on Australian tourism demand.  Overall, the method of bias-correcting before carrying out reconciliation is shown to outperform alternatives that only bias-correct or only reconcile forecasts.
...
\begin{table}
	\caption {Average {MSE($\times 10^3$)} of $1$-step-ahead point forecasts from log and Box-Cox transformed series. Biased denotes forecasts from simply reversing the transformation via \eqref{eq:BoxCox_back-transformation}. Unbiased(Method-1) performs bias adjustment via a Taylor series expansion as shown in \eqref{eq:BoxCox_BT_biasadj} whereas Unbiased(Method-2) bias adjusts by subtracting the in-sample forecast error mean.}
