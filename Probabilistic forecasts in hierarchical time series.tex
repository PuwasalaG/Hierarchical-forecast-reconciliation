...
\begin{flalign*}
  \E[(\bm{Y}_{T+h} - \hat{\bm{Y}}_{T+h})(\bm{Y}_{T+h} - \hat{\bm{Y}}_{T+h})'|\bm{\mathcal{I}}_T] = &  \E[(\bm{Y}_{T+h} - \E(\bm{Y}_{T+h}|\bm{\mathcal{I}}_T) + \E(\bm{Y}_{T+h}|\bm{\mathcal{I}}_T)- \hat{\bm{Y}}_{T+h})\\
...
\begin{flalign}
  Ss[S_B(.)] = \frac{S_B(\bm{{{Y}},y})^{ref} - S_B(\bm{{\breve{Y}},y})}{S_B(\bm{{{Y}},y})^{ref}}\times 100\%,
\end{flalign}
...
\begin{table}
  \caption{Comparison of incoherent vs. coherent forecasts for the individual bottom level series using Skill score.}
