...
\begin{proof}
	...
	\end{align}
	The Cauchy-Schwarz inequality can be used to show that orthogonal projections are bounded operators\todo{Hunter}, therefore
	 \begin{equation*}
...
\end{equation}
where $\breve{\bm{\mu}}_{T+h} =\E_{\bm{F}}(\breve{\bm{Y}}_{T+h}) $. In this limiting case, the energy score only measures the accuracy of the forecast mean (not the entire distribution), and the energy score is proper but not strictly proper. \citet{Pinson2013a} also argue that the energy score has low discriminative ability for incorrectly specified covariances, even though it discriminates the misspecified means well.
