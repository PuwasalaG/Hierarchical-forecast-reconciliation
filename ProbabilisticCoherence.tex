...
\begin{definition}[Coherent Probabilistic Forecasts]\label{def:cohprob}
  Let $(\mathbb{R}^m, \mathscr{F}^{\mathbb{R}^m}, \nu)$ be a probability triple, where $\mathscr{F}^{\mathbb{R}^m}$ is the usual $\sigma$-algebra on $\mathbb{R}^m$. Let $\breve{\nu}$ be a probability measure on $\mathfrak{s}$ with $\sigma$-algebra $\mathscr{F}^{\mathfrak{s}}$.  Here $\mathscr{F}^{\mathfrak{s}}$ is formed as collection of sets $s(B)$, where $s(B)$ denotes the image of the set $B\in \mathscr{F}^{\mathbb{R}^m}$ under the mapping $s(.)$.  Let the measure $\breve{\nu}$ have the property
  $$
      \breve{\nu}(s(B)) = \nu(B) \quad \forall  B \in \mathscr{F}^{\mathbb{R}^m},
    $$  
...
%For probabilistic forecasts, reconciliation implies finding the probability measure of the coherent forecasts using the information from an incoherent probabilistic forecast measure. A more formal definition is given below.
  ...
  \begin{equation}
  \nu(B) = \hat{\nu}(g^{-1}(B)), \quad \forall \quad B \in \mathscr{F}^{\mathbb{R}^m}\,.
  \end{equation}
  Here $g^{-1}(B):=\left\{\bm{y}\in \mathbb{R}^n:g(\bm{y})\in B \right\}$ is the pre-image of $B$, that is the set of all points in $\mathbb{R}^n$ that $g(.)$ maps to a point in $B$.
  ...
  \begin{equation}
  \tilde{\nu}(s(B)) = \nu(B)= \hat{\nu}(g^{-1}(B)) \qquad \forall B \in \mathscr{F}^{\mathbb{R}^m}\,.
  \end{equation}
\end{definition}

We now discuss how this definition can be used in practice to obtain reconciled probabilistic forecasts for hierarchical time series.   Recall that the case of forecast reconciliation could be broken down into three steps.  In the first via a change of basis $\hat{\bm{y}}_{t+h|t}$ is transformed into coordinates $\tilde{\bm{b}}_{t+h|t}$ and $\tilde{\bm{a}}_{t+h|t}$.  In the second, $\tilde{\bm{a}}_{t+h|t}$ is discarded and $\tilde{\bm{b}}_{t+h|t}$ are kept as the bottom level reconciled forecasts.  In the third, reconciled forecasts for the entire hierarchy are recovered via $\tilde{\bm{y}}_{t+h|t}=\bm{S}\tilde{\bm{b}}_{t+h|t}$.  We now outline these three steps for probabilistic forecasts and demonstrate how they correspond to Definition~\ref{def:reconprob}.

To clarify, we note that while $\hat{\nu}$ is a probability measure for an $n$-vector $\hat{\bm{y}}$,  probability statements in terms of a different coordinate system can be made via an appropriate change of basis. Letting $f(.)$ be generic notation for a probability density functions and following the notation from our definition of point forecast reconciliation where $\hat{\bm{y}}=\bm{S}\tilde{\bm{b}}+\bm{R}\tilde{\bm{a}}$ we obtain

\begin{equation}
f(\hat{\bm{y}})=f(\bm{S}\tilde{\bm{b}}+\bm{R}\tilde{\bm{a}})|\left(\bm{S}~\bm{R}\right)|
\end{equation}

The expression $\hat{\nu}(g^{-1}(B))$ is in Definition~\ref{def:reconprob} is equivalent to the probability statement $\mbox{Pr}(\hat{\bm{y}}\in g^{-1}(B))$.  After the change of basis this is equivalent to $\mbox{Pr}(\tilde{\bm{b}}\in (B))$ which implies

\begin{align}
\mbox{Pr}(\hat{\bm{y}}\in g^{-1}(B))&=\int_{g^{-1}(B)}f(\hat{\bm{y}})d\hat{\bm{y}}\\
&=\int_{B}\int f(\bm{S}\tilde{\bm{b}}+\bm{R}\tilde{\bm{a}})|\left(\bm{S}~\bm{R}\right)|d\tilde{\bm{a}}d\tilde{\bm{b}}
\end{align}

After integration out over $\tilde{\bm{a}}$, a step analogous to setting $\bm{a}=0$ for point forecasting we obtain a expressions that gives the probability the bottom level (reconciled) series lie in the region $B$, which corresponds to $\nu(B)$ in Definition~\ref{def:reconprob}.  To make a valid probability statement about the entire hierarchy we simply use the bottom level probabilistic forecasts together with Definition~\ref{def:cohprob}. 
...
Suppose an unreconciled probabilistic forecast is Gaussian with mean $\hat{\bm \mu}$ and variance-covariance matrix $\hat{\bm \Sigma}$.  The subscripts $t+h|t$ are surpresed for brevity.  The unreconciled density
...
\begin{equation}
f(\tilde{\bm b},\tilde{\bm a})=(2\pi)^{-\frac{n}{2}}\Big|\bm{\hat{\Sigma}}_{t+h}\Big|^{-\frac{1}{2}}\Big|(\bm{S} ~ \vdots~ \bm{R})\Big|\exp\left\{-\frac{1}{2}q\right\}\,,
\end{equation}
where
\begin{equation}
...
\begin{align*}
q& = 
\Big((\bm{S} ~ \vdots~ \bm{R})\bt-\bm{\hat{\mu}}\Big)' \bm{\hat{\Sigma}}^{-1}\Big((\bm{S} ~ \vdots~ \bm{R})\bt-\bm{\hat{\mu}}\Big),\\
& =
\Big(\bt-(\bm{S} ~ \vdots~ \bm{R})^{-1}\bm{\hat{\mu}}_{t+h}\Big)' \Big[(\bm{S} ~ \vdots~ \bm{R})\bm{\hat{\Sigma}_{t+h}}(\bm{S} ~ \vdots~ \bm{R})'\Big]^{-1}
\Big(\bt-(\bm{S} ~ \vdots~ \bm{R})^{-1}\bm{\hat{\mu}}_{t+h}\Big)\,.
\end{align*}
...
\end{pmatrix},
$$
where $\bm{G}=(\bm{R}'_\bot \bm{S})^{-1}\bm{R}'_\bot$ and $\bm{H}=(\bm{S}'_\bot \bm{R})^{-1}\bm{S}'_\bot$. Then $q$ can be simplified further as
\begin{align*}
q& =%\frac{1}{(2\pi)^{\frac{n}{2}}\Big|\bm{\hat{\Sigma}}_{t+h}\Big|^{\frac{1}{2}}\Big|\PQ \Big|}
%\exp \Big\{-\frac{1}{2}
\Big[\bt-\PQ\bm{\hat{\mu}}_{t+h}\Big]'%\\[-0.5cm]
\Big[\PQ\bm{\hat{\Sigma}_{t+h}}\PQ'\Big]^{-1}\Big[\bt-\PQ\bm{\hat{\mu}}_{t+h}\Big] %\Big\},
\\[0.5cm]
...
%& \hspace*{7cm}
 \Big[\PQ\bm{\hat{\Sigma}_{t+h}}\PQ'\Big]^{-1}\begin{pmatrix}\tilde{\bm{b}} - \bm{G}\bm{\hat{\mu}}\\ \tilde{\bm{a}}- \bm{H}\bm{\hat{\mu}}\end{pmatrix} %\Big\}.
\end{align*}
...
\end{pmatrix}
\Big|^{-\frac{1}{2}}
\exp \Big\{-\frac{1}{2} \begin{pmatrix}\tilde{\bm{b}} - \bm{G}\bm{\hat{\mu}}\\ \tilde{\bm{a}}- \bm{H}\bm{\hat{\mu}}\end{pmatrix}'\\
&\hspace*{7cm}
\begin{pmatrix}
\bm{G}\bm{\hat{\Sigma}}\bm{G}' & \bm{G}\bm{\hat{\Sigma}}\bm{H}' \\
\bm{H}\bm{\hat{\Sigma}}\bm{G}' & \bm{H}\bm{\hat{\Sigma}}\bm{H}'
\end{pmatrix}^{-1}
\begin{pmatrix}\tilde{\bm{b}} - \bm{G}\bm{\hat{\mu}}\\ \tilde{\bm{a}}- \bm{H}\bm{\hat{\mu}}\end{pmatrix} \Big\}.
\end{align*}
Marginalising out $\tilde{\bm{a}}$, it follows leads to the following bottom level reconciled forecasts. 
...
%\end{equation}
Which implies that the reconciled probabilistic forecast for the bottom level series is $\tilde{\bm{b}}_{t+h} \sim \mathcal{N}(\bm{P}\bm{\hat{\mu}}_{t+h}, \bm{P}\hat{\bm{\Sigma}}_{t+h}\bm{P}')$.  The reconciled probabilistic forecasts for the whole hierarchy follow a degenerate Gaussian distribution with mean  $\bm{SG}\bm{\hat{\mu}}$ and rank deficient covariance matrix $\bm{SP}\hat{\bm{\Sigma}}_{t+h}\bm{P}'\bm{S}'$.
...
\begin{theo}[Reconciliation for Elliptical Distributions]
	Let an unreconciled probabilistic forecast come from the elliptical class with location parameter $\hat{\bm \mu}$ and scale matrix $\hat{\bm \Sigma}$.  Let the true predictive distribution of ${\bm y}_{t+h|t}$ also belong to the elliptical class with location parameter ${\bm \mu}$ and scale matrix ${\bm \Sigma}$.  Then the affine reconciliation mapping $g({\bm y})={\bm G}{\bm y}+{\bm d}$ with $\bm{G}={\bm\Omega}^{1/2}{\bm\Sigma}^{-1/2}$ and ${\bm d}={\bm S}{\bm G}\left({\bm \mu}-\hat{\bm \mu}\right)$ recovers the true predictive density, where $\bm\Omega$ is the true variance covariance matrix of the predictive distribution for the bottom level. 
\end{theo}
\begin{proof}
   Since elliptical distributions are closed under affine transformations, and are closed under marginalisation, reconciliation of an elliptical distribution yields an elliptical distribution (although the unreconciled and unreconciled distributions may be different members of the class of elliptical distributions).  The scale matrix of the reconciled forecast is given by $\bm{S}\bm{G}{\bm \Sigma}{\bm G}'{\bm S}'$ while the location matrix is given by $\bm{S}\bm{G}\hat{\bm \mu}+{\bm d}$.  For the choice of ${\bf G}$ given above, the reconciled scale matrix is 
   \begin{align*}
   ...
   =&{\bm \mu}\\
   \end{align*}
   The final equality above holds since ${\bm S}{\bm G}$ projects any vector onto the coherent subspace, however since ${\bm \mu}$ is the mean of the true data generating process, it must already lie on the coherent subspace.
\end{proof}	
It should be noted that these choices of ${\bm G}$ and ${\bm d}$ depend on the true location and scale of the predictive distribution and are thus infeasible in practice.  However, these expressions do provide some insight on why some choices for ${\bm G}$ in the point forecast reconciliation literature may also work well for probablistic forecasts.  To illustrate, first rearrange the equation for the optimal value of  $\bm{G}_{opt}={\bm\Omega}^{1/2}\hat{\bm\Sigma}^{-1/2}$ as ${\bm\Omega}^{1/2}=\bm{G}_{opt}\hat{\bm\Sigma}^{1/2}$. Using an arbitrary ${\bm G}$ provides an approximation for ${\bm\Omega}^{1/2}$.  This approximation is equivalent to the way bottom level estimates are produced in point forecast reconciliation, however it is carried out columnwise on  $\hat{\bm\Sigma}^{1/2}$.  Meanwhile the value for $\tilde{\bm \mu}_{opt}$ includes a projection of $\hat{\bm \mu}$ onto $\mathfrak{s}$, and while it is infeasible to obtain the translation ${\bm d}$, it is worthwhile noting that this measures the distance between the reconciled mean and true mean. 
