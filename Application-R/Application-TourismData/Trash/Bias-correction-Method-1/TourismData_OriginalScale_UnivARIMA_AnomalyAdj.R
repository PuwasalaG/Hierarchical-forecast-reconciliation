...
for (j in 1:140) {#C = 140
  ...
  for (i in 1:n) {
    ...
    Base_forecasts_unbiased[,i] <- Fc$mean
    
    # Fitted_BackTrans <- mapply(Fc_backtransformed, fit_trans$fitted, 
    #                            sigma2 = var(fit_trans$residuals), lambda = lambda)
    
    
    # Residuals_BackTransformed[,i] <- as.vector(TS - fit$fitted)
    Residuals_BackTransformed_biasadjust[,i] <- as.vector(TS - fit$fitted)
  ...
  #--Reconciling bias-adjusted base forecasts--#
...
save.image("TourismData_OriginalScale_UnivARIMA_AnomalyAdj.RData")
...
DF %>%   
  ...
  summarise(MSE = mean(SquaredE)) %>%   
  filter(`F-method`=="ARIMA_unbias", `R-method`%in% c("Base", "OLS","WLS", "MinT(Shrink)")) %>%   
...
DF_MSE <- DF %>% 
  ...
  group_by(`F-method`, `R-method`, Replication, Forecast_Horizon) %>% 
  summarise(MSE = mean(SquaredE)) %>% 
  mutate(Forecast_Horizon = recode(Forecast_Horizon, "1" = "h=1", 
                                   "2" = "h=2", "3"= "h=3", "4" = "h=4", 
                                   "5" = "h=5", "6" = "h=6"))
