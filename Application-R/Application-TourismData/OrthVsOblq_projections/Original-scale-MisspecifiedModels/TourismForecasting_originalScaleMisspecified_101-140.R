...
for (j in 101:140) {#C=140
  ...
  fit <- Train %>% 
    group_by(Series) %>% 
    model(ARIMA = ARIMA(Overnight_Trips ~ PDQ(0,0,0)))
...
write.csv(x=Final_DF, file = "Results/DF_OriginalScaleMisspecified_101-140.csv")
save.image("Results/TourismForecasting_originalScaleMisspecified_101-140.RData")
