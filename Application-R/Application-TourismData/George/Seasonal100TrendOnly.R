...
H <- 12
L <- 80 #Size of the training sample
C <- nrow(AllTS_new) - L
j=100
for (j in 100:C) {#C = 140
  print(j)
  ...
  # WLS_sd G (This G matrix is calculated using back-transformed residuals)
  Inv_WLS_sd <- diag(1/sqrt(diag(var(Residuals_Base))), n, n)

  WLS_sd_G <- solve(t(S) %*% Inv_WLS_sd %*% S) %*% t(S) %*% Inv_WLS_sd
  ...
  Recon_WLS_sd <- t(S %*% WLS_sd_G %*% t(Base_forecasts))
  ...
  cbind(Fltr, Forecasts = as.vector(Recon_WLS_sd), "R-method" = "WLS_sd") -> Df_WLS_sd
  Df_WLS_sd[names(DF)] -> Df_WLS_sd
  DF <- rbind(DF, Df_WLS_sd)
...
save.image("Seasonal100TrendOnly.RData")
