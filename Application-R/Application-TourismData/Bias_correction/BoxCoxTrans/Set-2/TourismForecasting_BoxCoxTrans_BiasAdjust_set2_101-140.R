...
#Box-Cox transformation lambda is selected from Guerrero method constrained in (-0.5,2)
...
for (j in 101:140) {#C=140
  ...
  for (i in 1:n) {
    ...
    fit_bias <- auto.arima(TS_adj, lambda = lambda, biasadj = FALSE)
    fit_unbias <- auto.arima(TS_adj, lambda = lambda, biasadj = TRUE)
    
    #--Biased and Biased adjusted_M1 forecasts--#
    
    Base_forecasts_biased[,i] <- forecast::forecast(fit_bias, h=min(H, nrow(Test)), 
                                                    biasadj = FALSE)$mean - 1 #removing the 1 we added initially
    Base_forecasts_unbiased_M1[,i] <- forecast::forecast(fit_unbias, h=min(H, nrow(Test)), 
  ...
  targ_bias <- lowerD(na.omit(Residuals_Bias[[1]]))
  ...
  targ_unbiased_M1 <- lowerD(na.omit(Residuals_Unbiased_M1))
  ...
  targ_unbiased_M2 <- lowerD(na.omit(Residuals_Unbiased_M2[[1]]))
...
write.csv(x=DF, file = "Results/DF_BoxCoxTrans_101-140.csv")
save.image("Results/TourismData_BoxCoxTransBiasCorrection_101-140.RData")
