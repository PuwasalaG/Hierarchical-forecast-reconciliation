...
# Here we fit univariate ARIMA models for the Box-Cox transformed data
# The back transformed data will be biased and we bias adjust to get 
# unbiased point forecasts.

#Box-Cox transformation lambda is selected from Guerrero method limited between (-0.5,2)

#We use two methods for bias correction
# M1 - Bias correction via formulation
# M2 - Bias correction via residual mean adjustment

#This analysis takes a rolling window approach and we use window length as 180
...
H <- 12
L <- 180 #Size of the training sample 
C <- nrow(AllTS) - L
...
for (j in 1:C) {#C=60
  ...
  # lambda <- Train %>% 
  #   features(Overnight_Trips, features = guerrero, lower = 0, upper = 1) %>% 
  ...
  for (i in 1:n) {
    ...
    lambda <- TS_adj %>% 
      BoxCox.lambda(method = "guerrero", lower = -0.5, upper = 2)
    
    fit_bias[[i]] <- auto.arima(TS_adj, lambda = lambda, biasadj = FALSE)
    fit_unbias[[i]] <- auto.arima(TS_adj, lambda = lambda, biasadj = TRUE)
    
    #--Biased and Biased adjusted_M1 forecasts--#
    
    Base_forecasts_biased[,i] <- forecast::forecast(fit_bias[[i]], h=min(H, nrow(Test)), 
                                                    biasadj = FALSE)$mean - 1 #removing the 1 we added initially
    Base_forecasts_unbiased_M1[,i] <- forecast::forecast(fit_unbias[[i]], h=min(H, nrow(Test)), 
    ...
    for (h in 1:min(H, nrow(Test))) {
      ...
      Fitted <- fitted(fit_bias[[i]], h=h) - 1
      Residuals_Bias[[h]][,i] <- as.numeric(TS - Fitted)
      
    }
    
    Fitted_biasAdj <- fitted(fit_unbias[[i]], h=1) - 1
  ...
  targ_bias <- diag(diag(var(na.omit(Residuals_Bias[[1]]))),n,n)
  shrink_bias <- shrink.estim(na.omit(Residuals_Bias[[1]]), targ_bias)
  ...
  Inv_WLS_bias <- diag(1/diag(var(na.omit(Residuals_Bias[[1]]))), n, n)
  ...
  targ_unbiased_M1 <- diag(diag(var(na.omit(Residuals_Unbiased_M1))),n,n)
  shrink_unbiased_M1 <- shrink.estim(na.omit(Residuals_Unbiased_M1), 
                                     targ_unbiased_M1)
  ...
  Inv_WLS_unbiased_M1 <- diag(1/diag(var(na.omit(Residuals_Unbiased_M1))), n, n)
  ...
  targ_unbiased_M2 <- diag(diag(var(na.omit(Residuals_Unbiased_M2[[1]]))),n,n)
  shrink_unbiased_M2 <- shrink.estim(na.omit(Residuals_Unbiased_M2[[1]]), 
                                     targ_unbiased_M2)
  ...
  Inv_WLS_unbiased_M2 <- diag(1/diag(var(na.omit(Residuals_Unbiased_M2[[1]]))), n, n)
...
write.csv(x=DF, file = "Results/DF_BoxCoxTrans_WL100_1-50.csv")
save.image("Results/TourismData_BoxCoxTransBiasAdjust_WL100_1-50.RData")
