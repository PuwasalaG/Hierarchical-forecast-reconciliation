...
for (j in 101:140) {#C=140
  ...
  for (i in 1:n) {
    ...
    fit_bias <- auto.arima(TS, lambda = "auto", allowdrift = F, biasadj = FALSE)
    fit_unbias <- auto.arima(TS, lambda = "auto", allowdrift = F, biasadj = TRUE)
    ...
    for (h in 1:min(H, nrow(Test))) {
      ...
      Fitted <- fitted(fit_bias, h=h)
      Residuals_Bias[[h]][,i] <- as.numeric(TS - Fitted)
      
    }
    
    Fitted_biasAdj <- fitted(fit_unbias, h=1)
