...
for (j in 43:50) {#C=60
  ...
  targ_bias <- diag(diag(var(Residuals_Bias[[1]])))
  ...
  targ_unbiased_M1 <- diag(diag(var(Residuals_Unbiased_M1)))
  ...
  targ_unbiased_M2 <- diag(diag(var(Residuals_Unbiased_M2[[1]])))
...
write.csv(x=DF, file = "Results/DF_LogTrans_WL180_43-50.csv")
save.image("Results/TourismData_LogTransBiasAdj_WL180_43-50.RData")
