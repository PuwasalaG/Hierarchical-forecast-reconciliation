...
We can see this variable has a very unusual observation which might be an outlier. To avoid this, I have replaced this observation with the average of it's adjacent observations. Then I have refitted the models for automatically chosen BoxCox transformed data and back-transformed and back-transformed bias-adjusted forecasts were obtained. The results are as follows. 

```{r echo=FALSE}
load("TourismData_BoxCoxTransBiasCorrection_UnivARIMA_new.RData")
...
DF_BoxCox_2 %>% 
  group_by(`F-method`, `R-method`, Forecast_Horizon) %>% 
  summarise(MSE = mean(SquaredE)) -> Avg_MSE_BC_2
...
These results also shows that, even though the bias-adjusted based forecasts are worst than that of biased base forecasts, the MinT reconicilation is outperforming.
...
DF_Log %>% 
  group_by(`F-method`, `R-method`, Forecast_Horizon) %>% 
  summarise(MSE = mean(SquaredE)) -> Avg_MSE_Log
