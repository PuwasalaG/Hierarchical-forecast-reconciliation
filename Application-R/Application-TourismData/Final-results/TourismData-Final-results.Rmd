...
# Box and whisker plots for orthogonal and oblique projections
...
# More analysis on BoxCox transformed results
...
# DF_BoxCoxTrans %>% 
#   filter(Forecast_Horizon == 1, Series == "A") -> DF_BoxCoxTrans_A_h1
# 
# DF_BoxCoxTrans %>% 
#   filter(Forecast_Horizon == 1, Series == "B") -> DF_BoxCoxTrans_B_h1
...
DF_BoxCoxTrans_Tot_h1 %>% 
  ...
  ggtitle("Forecasts for Total series follow from BoxCox Transformation") + 
...
#   dplyr::select(F.method, Forecasts, Actual, Replication) %>% 
#   spread(key = F.method, value = Forecasts) %>% 
#   gather(-Replication, key = "Forecasts/Actual", value = Val) -> DF_BoxCoxTrans_A_h1
#   
# DF_BoxCoxTrans_A_h1 %>% 
#   mutate(`Forecasts/Actual` = recode(`Forecasts/Actual`, "Unbiased_M1" = "Unbiased")) %>%   ggplot() + 
#   geom_line(aes(x = Replication, y = Val, color = `Forecasts/Actual`)) + 
#   ylab("Forecasts/Actual") + 
#   ggtitle("Forecasts for NSW series follow from BoxCox Transformation") + 
#   theme(legend.position="bottom", legend.title = element_blank()) 
#  
#  DF_BoxCoxTrans_B_h1 %>% 
#   filter(R.method == "Base", F.method %in%c("Bias","Unbiased_M1")) %>%
...
#   dplyr::select(F.method, Forecasts, Actual, Replication) %>% 
#   spread(key = F.method, value = Forecasts) %>% 
#   gather(-Replication, key = "Forecasts/Actual", value = Val) -> DF_BoxCoxTrans_B_h1
#   
# DF_BoxCoxTrans_B_h1 %>% 
#   mutate(`Forecasts/Actual` = recode(`Forecasts/Actual`, "Unbiased_M1" = "Unbiased")) %>%   ggplot() + 
#   geom_line(aes(x = Replication, y = Val, color = `Forecasts/Actual`)) + 
#   ylab("Forecasts/Actual") + 
#   ggtitle("Forecasts for Victoria series follow from BoxCox Transformation") + 
#   theme(legend.position="bottom", legend.title = element_blank()) 
...
DF_BoxCoxTrans_Tot_h1 %>% 
  spread(key = `Forecasts/Actual`, value = Val) %>% 
  mutate("Unb-Act" = Unbiased_M1 - Actual) %>% 
  filter(`Unb-Act`>1000)

```

Very large bias corrected forecasts are given for some replications. For example in the rolling window Jul-2003 to Oct-2011. Observing the model auto.arima fits: 
...
TS <- Train[,"Total"] %>% ts(frequency = 12)
fit <- auto.arima(TS, lambda = "auto", biasadj = FALSE, stepwise = FALSE, approximation = FALSE)
Fc <- forecast(fit, h=1)
...
The estimated drift term has a very large standard error. Further the variance of $\hat{y}_{t+1}$ is `r sig2_h` which is very large
