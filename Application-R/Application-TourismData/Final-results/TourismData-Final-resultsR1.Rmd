...
# Log Transformation with 180 observations of training window (ME and MSE calculated over 60 replications)

```{r, echo=FALSE}
DF_LogTrans_WL180 <- read.csv("DF_LogTrans_WL180.csv")[,-1]

DF_LogTrans_WL180 <- DF_LogTrans_WL180 %>% 
  mutate(F.method=recode(F.method, `Bias` = 'Biased')) 
# so that we don't confuse Biased forecasts with ME


DF_LogTrans_WL180 %>% mutate(SquaredE = (`Actual` - `Forecasts`)^2) -> DF_LogTrans_WL180
DF_LogTrans_WL180 %>% mutate(ME = `Actual` - `Forecasts`) -> DF_LogTrans_WL180
...
DF_LogTrans_WL180 %>% 
  ...
  filter(Forecast_Horizon == 1) %>% 
  ungroup() %>% 
  dplyr::select(-Forecast_Horizon)-> Avg_Log_WL180_ME_h1
...
# BoxCox Transformation for $\lambda \in (-0.5,2)$ with 180 observations of training window (ME and MSE calculated over 60 replications)


```{r, echo=FALSE}
DF_BoxCoxTrans_WL180 <- read.csv("DF_BoxCoxTrans_WL180.csv")[,-1]

DF_BoxCoxTrans_WL180 <- DF_BoxCoxTrans_WL180 %>% 
  mutate(F.method=recode(F.method, `Bias` = 'Biased')) 
# so that we don't confuse Biased forecasts with ME

DF_BoxCoxTrans_WL180 %>% mutate(SquaredE = (`Actual` - `Forecasts`)^2) -> DF_BoxCoxTrans_WL180
DF_BoxCoxTrans_WL180 %>% mutate(ME = `Actual` - `Forecasts`) -> DF_BoxCoxTrans_WL180
