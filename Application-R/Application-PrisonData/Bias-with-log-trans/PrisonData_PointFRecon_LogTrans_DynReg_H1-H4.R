...
# For j=3, the models gives weired forecasts. So it was ignored
  ...
  #-- For aggregate series we fit univariate ARIMA models with transformation --#
    ...
    Base_forecasts_biased[,i] <- forecast(fit_dyn.reg_bias, h=min(H, nrow(Test)), xreg = xreg_newdata, lambda = 0, biasadj = FALSE)$mean
    Base_forecasts_unbiased[,i] <- forecast(fit_dyn.reg_unbias, h=min(H, nrow(Test)), xreg = xreg_newdata, lambda = 0, biasadj = TRUE)$mean
