...
# For aggregate series we fit time series regression with ARIMA errors for the log transformed data. 
...
# For bottom level series we fit univariate ARIMA models with log transformations
...
for (j in c(2,4:20)) {#C 
  ...
  for (i in 1:(n-m)) {
    ...
    #-- For aggregate series we fit univariate ARIMA models with transformation --#
      ...
      TS <- Train[,i]
      fit_bias <- auto.arima(TS, lambda = 0, biasadj = FALSE) #This will give back-transformed fitted values
      fit_unbiased <- auto.arima(TS, lambda = 0, biasadj = TRUE) #This will give back transformed bias-adjusted fitted values
      
      Base_forecasts_biased[i] <- forecast(fit_bias, h=H, biasadj = FALSE)$mean 
      Base_forecasts_unbiased[i] <- forecast(fit_unbiased, h=H, biasadj = TRUE)$mean 
    ...
    # Y <- Train[,i]
    # Data_log <- data.frame(Y, Bts_lag_tr) %>% 
    #   log() %>% 
    #   as.ts()
    # 
    # xname <- colnames(Bts_lag_tr)
    ...
    #   
    # Residuals_BackTransformed[,i] <- as.vector(Y - exp(tslm_fit$fitted))
    # Residuals_BackTransformed_biasadjust[,i] <- as.vector(Y - exp(tslm_fit$fitted))
